...
class MultitaskRerankingDataset:
    ...
    def __getitem__(self, item):
        ...
        summary_candidates_inputs = self.tokenizer(summary_candidates, return_tensors="pt", truncation=True, max_length=self.max_summary_length, padding='max_length')
        summary_candidates_inputs["input_ids"] = summary_candidates_inputs["input_ids"][:,:self.max_summary_length]
        summary_candidates_inputs["attention_mask"] = summary_candidates_inputs["attention_mask"][:,:self.max_summary_length]

        text_and_summaries = [self.tokenizer.decode(text_inputs["input_ids"][0], skip_special_tokens=True) + " " + self.sep_symbol + " " \
        ...
        text_and_summaries_inputs = self.tokenizer(text_and_summaries, return_tensors="pt", truncation=True, max_length=self.max_length + self.max_summary_length, padding='max_length')
        text_and_summaries_inputs["input_ids"] = text_and_summaries_inputs["input_ids"][:, :(self.max_length + self.max_summary_length)]
        text_and_summaries_inputs["attention_mask"] = text_and_summaries_inputs["attention_mask"][:, :(self.max_length + self.max_summary_length)]
